...
pub fn main() {
	...
	for instruction in instructions{
		...
		// Follow the instruction of moving from one pile to another the correct amount of times in groups
		// Create a temporary letters stack so that the order is not reversed
		let mut letters: Vec<&str> = Vec::new(); 
		for _amount in 0..instruction.amount{
			let letter: &str = piles[instruction.from].pop().unwrap();
			letters.push(letter);
		}
		for _letter in 0..letters.len(){
			piles[instruction.to].push(letters.pop().unwrap())
		}
	}
