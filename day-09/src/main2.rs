...
pub fn main() {
	...
	*/
	...
	let mut rope: [[isize; 2]; ROPE_SIZE] = [[0, 0]; ROPE_SIZE];

	let mut visited: HashMap<[isize; 2], usize> = HashMap::new();	
	visited.insert([0, 0], 1); // visit 0,0
	...
	for movement in movements {
		...
		for _i in 0..amount {
			...
			// Move the start
			rope[0] = vec_add(rope[0], direction);

			for j in 0..rope.len() - 1 {
			
				let difference = vec_rem(rope[j + 1], rope[j]);

				// Move the end if the distance is greater than 1 by 1 (root 2)
				if vec_len(difference) > 1.5 {
					rope[j + 1] = vec_add(rope[j + 1], vec_trim(difference));
				}
			}
			
			// Set the vec to visited
			// println!("{:?}", rope[10]);
			visited.insert(rope[rope.len() - 1], 1);
